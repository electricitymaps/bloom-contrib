...
export const explanation = {
  ...
  links: [
    ...
    {
      label: 'The Parliamentary Office of Science and Technology (2016)',
      href: 'http://researchbriefings.files.parliament.uk/documents/POST-PN-0523/POST-PN-0523.pdf',
    },
...
function carbonEmissionsWithoutParticipants(activity) {
  const { countryCodeISO2, heatingSource, energyWattHours } = activity;
  ...
  if (!energyFootprint.intensityKilograms) {
    throw new Error(
      `Unable to find a non-country-specific carbon intensity for heatingSource ${heatingSource}`
    );
  }
